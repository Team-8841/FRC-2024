...
public class SimSwerveModuleIO implements SwerveModuleIO {
  DCMotorSim steeringMotor =
      new DCMotorSim(
          SimConstants.steeringGearbox,
          SimConstants.steeringGearRatio,
          SimConstants.steeringInertia);
  DCMotorSim driveMotor =
      new DCMotorSim(
          SimConstants.driveGearbox, SimConstants.driveGearRatio, SimConstants.driveInertia);

  PIDController steeringPID =
      new PIDController(SimConstants.steeringKP, SimConstants.steeringKI, SimConstants.steeringKD);

  SimpleMotorFeedforward driveFeedForward = new SimpleMotorFeedforward(0, SimConstants.driveKV);
  PIDController drivePID =
      new PIDController(SimConstants.driveKP, SimConstants.driveKI, SimConstants.driveKD);
  ...
  public void periodic() {
    double steeringPower =
        this.steeringPID.calculate(
            Units.radiansToDegrees(this.steeringMotor.getAngularPositionRad()));
    ...
    double drivePower =
        this.drivePID.calculate(this.driveMotor.getAngularVelocityRadPerSec()) + driveFeedForward;
  ...
  public SwerveModuleState getState() {
    double speedMetersPerSecond =
        this.rad2WheelSpeed(this.driveMotor.getAngularVelocityRadPerSec());
